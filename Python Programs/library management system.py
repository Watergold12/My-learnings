...
class LibraryManagementSystem:
        ...
        if title and author and genre and quantity:
            try:
                ...
                if title in self.books:
                    ...
                    self.books[title] = {"Author": author, "Genre": genre, "Quantity": quantity, "Available": quantity}
                self.update_table()
    ...
    def search_book(self):
        ...
        for idx, (book_title, details) in enumerate(self.books.items(), start=1):
            if book_title.lower().startswith(title.lower()) and details["Available"] > 0:
                self.table.insert("", "end", text=idx, values=(book_title, details["Author"], details["Genre"], details["Quantity"], details["Available"]))
    ...
    def update_table(self):
        ...
        for idx, (title, details) in enumerate(self.books.items(), start=1):
            self.table.insert("", "end", text=idx, values=(title, details["Author"], details["Genre"], details["Quantity"], details["Available"]))
...
def main():
    ...
    root.geometry("800x400")  # Adjust the window size to a more user-friendly dimension
    root.mainloop()

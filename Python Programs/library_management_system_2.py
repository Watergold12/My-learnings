...
class LibraryManagementSystem:
    def __init__(self, master):
        ...
        self.label_desc = tk.Label(master, text="Description:")
        self.label_desc.grid(row=4, column=0, padx=10, sticky="e")
        self.entry_desc = tk.Entry(master)
        self.entry_desc.grid(row=4, column=1, padx=10, sticky="w")
        ...
        self.table = ttk.Treeview(master, columns=("Title", "Author", "Genre", "Description", "Quantity", "Available"))
    ...
    def resize_table(self, event):
        ...
        self.table.column("Title", width=int(table_width * 0.2))
        self.table.column("Author", width=int(table_width * 0.2))
        self.table.column("Genre", width=int(table_width * 0.2))
        self.table.column("Description", width=int(table_width * 0.3))
        self.table.column("Quantity", width=int(table_width * 0.2))
    ...
    def add_book(self):
        ...
        if title and author and genre and desc and quantity:
            try:
                quantity = int(quantity)
                if title in self.books:
                    self.books[title]["Quantity"] += quantity
                    self.books[title]["Available"] += quantity
                else:
                    self.books[title] = {"Author": author, "Genre": genre, "Description": desc, "Quantity": quantity, "Available": quantity}
                self.update_table()
    ...
    def search_book(self):
        ...
        for idx, (book_title, details) in enumerate(self.books.items(), start=1):
            if book_title.lower().startswith(title.lower()) and details["Available"] > 0:
                self.table.insert("", "end", text=idx, values=(book_title, details["Author"], details["Genre"], details["Description"], details["Quantity"], details["Available"]))
    ...
    def update_table(self):
        ...
        for idx, (title, details) in enumerate(self.books.items(), start=1):
            self.table.insert("", "end", text=idx, values=(title, details["Author"], details["Genre"], details["Description"], details["Quantity"], details["Available"]))
...
def main():
    ...
    root.geometry("900x500")  # Adjust the window size to a more user-friendly dimension
    root.mainloop()
